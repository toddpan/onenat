#!/usr/bin/env bash
# =====================================================================
#  ngrokd 服务端启动脚本 (本地测试 / 生产通用)
#
#  本地测试:  bash start-ngrokd.sh                 (纯 IP, 免域名免证书)
#  生产部署:  sudo ./ngrokd-domain.com.crt ... 见文末"生产模式"说明
#             DOMAIN=你的域名 bash start-ngrokd.sh
#
#  常用环境变量:
...
#    LOGFILE     日志路径 (默认 ./ngrokd.log)
#    WEB_PORT    管理后台端口 (默认 18080; 设为空字符串则禁用)
#    WEB_DATA    管理后台数据文件 (默认 ./ngrokd-dashboard.json)
#    DL_DIR      客户端二进制分发目录 (默认 ./dl)
#  例:
#    bash start-ngrokd.sh                                    # 本地最小化
#    DOMAIN=ngrok.me HTTP_PORT=80 AUTH_TOKENS=k1,k2 bash start-ngrokd.sh
#    DOMAIN=ngrok.me TUNNEL_PORT=14443 HTTP_PORT=18080 bash start-ngrokd.sh
# =====================================================================
set -euo pipefail
cd "$(dirname "$0")"
...
TLS_CERT="${TLS_CERT-}"
TLS_KEY="${TLS_KEY-}"
LOGFILE="${LOGFILE-./ngrokd.log}"
PIDFILE="./ngrokd.pid"
CONSOLE_LOG="${CONSOLE_LOG-./ngrokd-console.log}"
# 管理后台 (web 管理页面): WEB_PORT 为空则禁用
WEB_PORT="${WEB_PORT-18080}"
WEB_DATA="${WEB_DATA-./ngrokd-dashboard.json}"
DL_DIR="${DL_DIR-./dl}"
WEB_ADMIN_PASS="${WEB_ADMIN_PASS-}"

# 已在运行则先停
if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
  echo "ngrokd 已在运行 (pid $(cat $PIDFILE)), 先停止..."
...
echo "✓ ngrokd 已启动 (pid $(cat $PIDFILE), 日志: $LOGFILE)"
...
# 初始密码: stdout 即时打印(ngrokd-console.log), 文件日志异步刷盘作为兜底
INITIAL_PASS=""
...
echo
echo "  生产模式(正式证书+标准口):"
echo "    sudo DOMAIN=你的域名 HTTP_PORT=80 HTTPS_PORT=443 TLS_CERT=/path/crt TLS_KEY=/path/key AUTH_TOKENS=k1,k2 bash start-ngrokd.sh"
