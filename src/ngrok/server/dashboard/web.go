...
set -e
# 可用环境变量 ONE_NAT_SERVER 覆盖 (http 入口被 nginx 占用 / 反代未传 X-Forwarded-Proto 时)
SERVER="${ONE_NAT_SERVER:-{{.BaseURL}}}"
TUNNEL_ID="${1:-}"; KEY="${2:-}"
...
# baked SERVER 是 http:// 时, 80 端口可能被反代/静态站占用, 自动尝试 https 同主机
download_to() { # $1=dest $2=path
  _dest="$1"; _path="$2"
  _alt=""
  case "$SERVER" in http://*) _alt="https://${SERVER#http://}$_path" ;; esac
  if command -v curl >/dev/null 2>&1; then
    curl -# -fSL "$SERVER$_path" -o "$_dest" || { [ -n "$_alt" ] && curl -fsSL "$_alt" -o "$_dest"; }
  elif command -v wget >/dev/null 2>&1; then
    wget --progress=bar:force -q --show-progress "$SERVER$_path" -O "$_dest" 2>&1 \
      || wget -q "$SERVER$_path" -O "$_dest" \
      || { [ -n "$_alt" ] && wget -q "$_alt" -O "$_dest"; }
  else
...
download_client() {
  download_to "$TMP/ngrok" "/dl/$BIN_NAME"
  chmod +x "$TMP/ngrok"
...
mkdir -p "$CFG_DIR"
download_to "$CFG_DIR/ngrok-managed.yml" "/api/deploy?id=$TUNNEL_ID&key=$KEY"
...
# 可用环境变量 ONE_NAT_SERVER 覆盖; baked http 失效时自动回退 https
$BaseURL = if ($env:ONE_NAT_SERVER) { $env:ONE_NAT_SERVER } else { "{{.BaseURL}}" }
$BaseURLAlt = ""
if ($BaseURL.StartsWith("http://")) { $BaseURLAlt = $BaseURL -replace "^http://", "https://" }
function Fetch-Url($Url, $Out) {
  try { Invoke-WebRequest -UseBasicParsing -Uri $Url -OutFile $Out }
  catch {
    if ($BaseURLAlt -and $Url.StartsWith($BaseURL)) {
      Invoke-WebRequest -UseBasicParsing -Uri ($BaseURLAlt + $Url.Substring($BaseURL.Length)) -OutFile $Out
    } else { throw }
  }
}
try {
...
if (-not $skip) {
  Fetch-Url "$BaseURL/dl/$exeName" "$exePath.tmp"
  if ($wantMd5) {
...
Fetch-Url "$BaseURL/api/deploy?id=$TunnelId&key=$Key" $cfgPath
